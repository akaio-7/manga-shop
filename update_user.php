...
if (isset($_POST['submit'])) {
    ...
    if ($name == '') {
        ...
        $update_name  = $conn->prepare("UPDATE users SET name= :name WHERE id=:uid");
        $update_name->bindParam(":uid",$user_id);
    ...
    // fetch old pass
    $select_old_pass = $conn->prepare("SELECT password FROM users WHERE id = :uid ");
    $select_old_pass->bindParam(":uid",$user_id);
    ...
    if( $old_pass == $empty_pass ) {
        ...
        if ( $new_pass != $empty_pass ) {
            $update_pass = $conn->prepare("UPDATE users SET password= :password WHERE id=:uid");
            $update_pass->bindParam(":uid",$user_id);
